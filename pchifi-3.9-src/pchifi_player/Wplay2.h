...
//#define	 MAX_BUFFER_SIZE         1048576      //wave out buffers size
#define  MAX_BUFFER_SIZE         262144
#define  SMG_LEN			     300          //string lenght
#define  HIFI_SIZE               70000000     //hifi file size
...
class pwave  // class added by daipozhi
{
  private:
    ...
    char    pwav_st10[HIFI_SIZE];
    char    pwav_st11[HIFI_SIZE];
    char    pwav_st12[HIFI_SIZE];

    char    pwav_st20[MAX_BUFFER_SIZE];
    char    pwav_st21[MAX_BUFFER_SIZE];
    char    pwav_st22[MAX_BUFFER_SIZE];

    char    pwav_st30[MAX_BUFFER_SIZE];
    char    pwav_st31[MAX_BUFFER_SIZE];
    char    pwav_st32[MAX_BUFFER_SIZE];
  ...
  public:
	...
	int       put_pwav_st10(long addr,int val);
	int       put_pwav_st11(long addr,int val);
	int       put_pwav_st12(long addr,int val);
	...
	int       get_pwav_st10(long addr);
	int       get_pwav_st11(long addr);
	int       get_pwav_st12(long addr);
	...
	long      load_hifi_files(char *play_file);

    int       dsp_msg_rst(HWND hwnd,int i);
	int       dsp_msg_cls(HWND hwnd,int i);

	int		  pwave_continue1(HWND hwnd);
	int		  pwave_continue2(HWND hwnd);
