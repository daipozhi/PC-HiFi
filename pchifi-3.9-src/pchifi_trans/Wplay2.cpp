// daipozhi modified for PC HiFi play 1998-2002
...
#include "fftsize.h"  // fft  size define
...
void pwave::wPlay_OnWOM_DONE(HWND hwnd, HWAVEOUT wavedevice, LPWAVEHDR lpwaveheader)
{
  ...
  if (pwave_end_pre>0)
  {
	...
	if ((lpwaveheader==pwavehdrinfo[0])||(lpwaveheader==pwavehdrinfo[1])||(lpwaveheader==pwavehdrinfo[2]))
	{
		if ((st3[0]==st3[1])&&(st3[0]==st3[2]))
		{
			...
			sprintf(str,"buffnums %ld,time(ms):%ld,%ld,%ld,",st3[0],st6[0],st6[1],st6[2]);
  ...
  else
  {
	if (pwave_end==1)
	{
	  ...
	  close(fh_f1);  // close transform data file
	  close(fh_f2);
	  close(fh_f3);
...
void pwave::wPlay_OnCommand(HWND hMainDlg, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	switch(id)  
	{
       ...
       case ID_RECORD: // bring up help box
       ...
       case ID_DELETE: // prepare all files
         ...
         if((l=SendDlgItemMessage(hMainDlg,ID_LISTBOX,LB_GETCURSEL,0,0L)) != LB_ERR) 
		 {
		   ...
		   { 
		     pwave_left=0;
             off_action(hwnd);
             SynTransWave((LPSTR)buffer,FNAMESIZE,hMainDlg);
		     on_action(hwnd);
		   }
		   else pwave_play=0;
         }
...
DWORD pwave::SynInitBuffer(HWND hwnd)
{
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++) pwav_st20[l]=0;
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++)  pwav_st21[l]=0;
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++)  pwav_st22[l]=0;
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++)  pwav_st30[l]=0;
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++)  pwav_st31[l]=0;
	...
	for (l=0;l<ALL_BUFFER_SIZE;l++)  pwav_st32[l]=0;
...
*/
/////////////////////////////////////////////////////////////////////
// Reads data from the file on the hard drive into the memory buffer pointed
// to by pwavehdr->lpData
// LPWAVEHDR pwavehdr	-	pointer to the WAVEHDR structure for memory buffer
// UINT *lastLoad - Not used any longer
// HWND hwnd	-	Main window handle
// Returns TRUE if successful FALSE for anything else
/////////////////////////////////////////////////////////////////////

DWORD  pwave::SynTransWave(char *fn,int fnsize,HWND hwnd)
{
  PCMWAVEFORMAT pcmwaveformat1;
  long          bytesRead;
  long          i;
  ...
  if(!OpenWaveFile(fn,hwnd,(LPPCMWAVEFORMAT)&pcmwaveformat1)) return FALSE;

  f1=_sopen(pf1,O_RDWR|O_CREAT|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  f2=_sopen(pf2,O_RDWR|O_CREAT|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  f3=_sopen(pf3,O_RDWR|O_CREAT|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);

  if ((f1<0)||(f2<0)||(f3<0)) return(1);

  eof=0;

  while (!eof)
  {
    bytesRead=mmioRead(hmmio,pwav_st40,ALL_BUFFER_SIZE);

    for (i=bytesRead;i<ALL_BUFFER_SIZE;i++) put_pwav_st40(i,0);

	if (bytesRead!=ALL_BUFFER_SIZE) eof=1;

    i=seper1.fft_transall(hwnd,pcm_num);
  
	if (i!=0) return(1);

	_write(f1,pwav_st41,ALL_BUFFER_SIZE);
	_write(f2,pwav_st42,ALL_BUFFER_SIZE);
	_write(f3,pwav_st43,ALL_BUFFER_SIZE);
  ...
  sprintf(str,"Differents rate: %20.10f...      ",pf_f1/pf_f2);
  MessageBox(hwnd,str,"Report",MB_OK);
...
DWORD pwave::SynLoadBuffer(HWND hwnd,long pcm,int bp,int chn)
{
   ...
   if (bp==0)
   {
	 if (chn==1) read(fh_f1,pwav_st20,MAX_BUFFER_SIZE);
	 if (chn==2) read(fh_f2,pwav_st21,MAX_BUFFER_SIZE);
	 if (chn==3) read(fh_f3,pwav_st22,MAX_BUFFER_SIZE);
	 if (eof(fh_f1)||eof(fh_f2)||eof(fh_f3)) pwave_end=1;
   ...
   {
	 if (chn==1) read(fh_f1,pwav_st30,MAX_BUFFER_SIZE);
	 if (chn==2) read(fh_f2,pwav_st31,MAX_BUFFER_SIZE);
	 if (chn==3) read(fh_f3,pwav_st32,MAX_BUFFER_SIZE);
	 if (eof(fh_f1)||eof(fh_f2)||eof(fh_f3)) pwave_end=1;
...
/////////////////////////////////////////////////////////////////////
/*
DWORD pwave::OpenWaveOut(PCMWAVEFORMAT pcmwaveformat, HWND hwnd)
{
	int  error;
	char buffer[STRINGSIZE+1];

	if((error=waveOutOpen((LPHWAVEOUT)&hwaveoutID,
						  WAVE_MAPPER,
...
/////////////////////////////////////////////////////////////////////
/*
DWORD pwave::PlayWaveData(HWND hwnd, LPWAVEHDR lpwavehdrbuf)
{
	int error;
	char buffer[STRINGSIZE+1];

	error=waveOutWrite(hwaveoutID,lpwavehdrbuf,sizeof(WAVEHDR));

	if (error)
	{
	  FreeBuffer(lpwavehdrbuf);
	  mmioClose(hmmio,0);
	  waveOutGetErrorText(error, (LPSTR)buffer, STRINGSIZE);
	  MessageBox(hwnd,buffer,"waveOutWrite",MB_OK|MB_ICONINFORMATION);
	  waveOutClose(hwaveoutID);
	  hwaveoutID = NULL;
	  return FALSE;
	}
...
{
    strcpy(szAppName,"Pc Windows Wave HiFi Wave Transforms");
    ...
    strcpy(help,"Buttons:\n");
    strcat(help,"    Prepare all - Prepare data before playback a wave file.\n");
    strcat(help,"    SynPlay     - Not implemented.\n");
    strcat(help,"    Stop Play   - Not implemented.\n");
    strcat(help,"    Pause       - Not implemented.\n");
    strcat(help,"    Previous    - Not implemented.\n");
    strcat(help,"    Next        - Not implemented.\n");
...
int pwave::put_pwav_st40(long addr,int val)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  pwav_st40[addr]=val;
  return(0);
}
int pwave::put_pwav_st41(long addr,int val)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  pwav_st41[addr]=val;
  return(0);
}
int pwave::put_pwav_st42(long addr,int val)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  pwav_st42[addr]=val;
  return(0);
}
int pwave::put_pwav_st43(long addr,int val)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  pwav_st43[addr]=val;
  return(0);
...
int pwave::get_pwav_st40(long addr)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  return(pwav_st40[addr]);
}
int pwave::get_pwav_st41(long addr)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  return(pwav_st41[addr]);
}
int pwave::get_pwav_st42(long addr)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  return(pwav_st42[addr]);
}
int pwave::get_pwav_st43(long addr)
{
  if ((addr<0)||(addr>=ALL_BUFFER_SIZE)) return(1);
  return(pwav_st43[addr]);
}
